...
async function getSongs(folder) {
    ...
    let songsApi = await fetch(`${folder}/list.json`)
    response = await songsApi.json()
    ...
    playMusic(response[0].replaceAll("%20"," ").split("-")[0], response[0].replaceAll("%20"," ").replace(".mp3","").split("-")[1], false, folder)
...
previous.addEventListener("click", ()=>{
    ...
    let index = response.findIndex(song => song === currentFile);
    console.log("Index is:", index);

    if((index - 1) == -1) {
        playMusic(response[response.length-1].replaceAll("%20"," ").split("-")[0], response[response.length-1].replaceAll("%20"," ").replace(".mp3","").split("-")[1], false, folder)
    }
    else{
        playMusic(response[index - 1].replaceAll("%20", " ").split("-")[0], response[index - 1].replaceAll("%20", " ").replace(".mp3", "").split("-")[1], false, folder)
    }
})
...
next.addEventListener("click", ()=>{
    ...
    let currentFile = currentSong.src.replaceAll("%20", " ").split("/").slice(-1)[0];
    console.log("Current file:", currentFile);
